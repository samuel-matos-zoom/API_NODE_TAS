...
exports.matriculasRoutes = (app) => {
    ...
    app.post('/setProgressoMatricula', (req, res) => {
        var sqlQry = [];
        var agora = moment().subtract({ hours: 3 }).format('YYYY-MM-DDTHH:mm:ss') + '.000Z';
        switch (req.body.porcentagem) {
            case "0":
                ...
                break;
            case "100":
                sqlQry = ["UPDATE curso_matricula SET progresso = ?, data_end = ?, status_curso = ?, WHERE id_usuario = ? AND id_curso = ? AND progresso < ?", [req.body.porcentagem, agora, 1, req.body.idUser, req.body.idCurso, 100]];
                break;
            default:
